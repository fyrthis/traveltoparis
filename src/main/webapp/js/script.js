                ...
                if(data.status=="success") {
                    ...
                    for(var i = 0; i < data.events.size; i++) {
                        ...
                        $('<div class="brdr bgc-fff pad-10 box-shad btm-mrg-20 property-listing expandable">' +
                            ...
                            '<a href="'+event.url+'" target="_parent"></a><h4 class="media-heading"><a href="'+event.url+'" target="_parent">'+event.name+'</a>' +
                            '</h4><ul class="list-inline mrg-0 btm-mrg-10 clr-535353"><li>FROM '+event.begins+'</li>' +
                            '<li style="list-style: none">|</li><li>TO '+event.ends+'</li>' +
                            '</ul><div><span class="pull-right"><button class="btn btn-primary">Add to trip</button></i></span></div>' +
    ...
    $(".btn-pref .btn.tab").on('click', function () {
        ...
        if($("#events").hasClass("btn-primary")){
            ...
            $(".btn-events").on('click', function () {
                var eventsbody = $('#spin1').parent();
                eventsbody.empty();
                eventsbody.append('<span id="spin1" class="glyphicon glyphicon-hourglass spin-loader" aria-hidden="true"></span>');
                callEventsTrip();
            });
        }
        //Tab Calendar
